...
function irr(cashflows::AbstractArray{<:Real, 1})
    ...
    try
        ...
        if e isa ArgumentError
            @debug "IRR root not bracketed on [0.0, $IRR_UPPER_RATE_BOUND] (npv@0=$(npv_at_zero), npv@$IRR_UPPER_RATE_BOUND=$(npv_at_upper), periods=$(length(cashflows))), returning 0.0 as conservative no-positive-IRR fallback"
            return 0.0
        end
        rethrow(e)
    end
