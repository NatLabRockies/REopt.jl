...
function outage_effective_production_factors(p)
    ...
    # MPCPV has no outage_production_fraction field, so only collect it where defined.
    pv_outage_production_fraction = Dict(pv.name => pv.outage_production_fraction
    ...
    for t in p.techs.elec
        ...
        if t in p.techs.pv
            factors[t] = factors[t] .* get(pv_outage_production_fraction, t, 1.0)
        end
    end
