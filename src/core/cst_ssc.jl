...
function run_ssc(case_data::Dict)
    if case_data["CST"]["tech_type"] == "novel"
        R = Dict{String, Any}()

        # Validation Check
        if length(case_data["CST"]["SSC_Inputs"]["thermal_production_series"]) != 8760
            error("Input error: thermal_production_series must have a length of 8760. Found: $(length(case_data["CST"]["SSC_Inputs"]["thermal_production_series"]))")
        end
        R["thermal_production_series"] = case_data["CST"]["SSC_Inputs"]["thermal_production_series"]
        R["electric_consumption_series"] = zeros(8760)
        R["error"] = "No errors found."
        return R
    end
    ################################################
    ### ALL CODE BELOW IS FOR SAM BASED MODELING ###
    ################################################
