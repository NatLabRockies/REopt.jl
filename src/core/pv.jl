...
```julia
    ...
    outage_prod_reduction_fraction::Real = 0.0, # Fractional reduction in production during outages. Only applies with multiple outage modeling using inputs outage_start_time_steps and outage_durations.
```
...
mutable struct PV <: AbstractTech
    ...
    outage_prod_reduction_fraction
    ...
    function PV(;
        ...
        if !(0.0 <= outage_prod_reduction_fraction <= 1.0)
            push!(invalid_args, "outage_prod_reduction_fraction must satisfy 0 <= outage_prod_reduction_fraction <= 1, got $(outage_prod_reduction_fraction)")
        end
