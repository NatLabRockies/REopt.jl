...
elseif "Dev" in ARGS
    ...
    @testset verbose = true "REopt test set using HiGHS solver" begin
        @testset "Sector defaults" begin
            ...
            for tech_struct in (s.pvs[1], s.wind, s.chps[1], s.steam_turbine)
            ...
            @test s.financial.om_cost_escalation_rate_fraction == 0.015
            @test s.financial.offtaker_discount_rate_fraction == 0.045
            for tech_struct in (s.pvs[1], s.wind, s.chps[1], s.ghp_option_list[1], s.steam_turbine)
        ...
        @testset "CHP Sizing Heuristic" begin
            ...
            case3_max_size_kw = response["chp_max_size_kw"]

            #Case 4: Verify Case 3 results match when creating full Scenario with CHP + AbsorptionChiller
            d = Dict{String, Any}(
                "Site" => Dict("latitude" => 35.0, "longitude" => -100.0),
                "ElectricLoad" => Dict("loads_kw" => repeat([1000.0], 8760), "year" => 2025),
                "ElectricTariff" => Dict("urdb_label" => "5ed6c1a15457a3367add15ae"),
                "SpaceHeatingLoad" => Dict(
                    "annual_mmbtu" => 100.0 * 8760 / REopt.KWH_PER_MMBTU,
                    "doe_reference_name" => "FlatLoad"
                ),
                "DomesticHotWaterLoad" => Dict(
                    "annual_mmbtu" => 0.0,
                    "doe_reference_name" => "FlatLoad"
                ),
                "CoolingLoad" => Dict("thermal_loads_ton" => repeat([100.0 / REopt.KWH_THERMAL_PER_TONHOUR], 8760)),
                "ExistingChiller" => Dict("cop" => 1.0),
                "ExistingBoiler" => Dict(
                    "fuel_cost_per_mmbtu" => 10.0,
                    "efficiency" => 0.8
                ),
                "AbsorptionChiller" => Dict(
                    "cop_thermal" => 1.0
                ),
                "CHP" => Dict(
                    "fuel_cost_per_mmbtu" => 10.0,
        ...
        @testset verbose = true "CHP" begin
            ...
            @testset "CHP Waste Heat, Absorption Chiller, Load Following Policies" begin
                ...
                results = run_reopt(m, d)
                @test sum(results["CHP"]["thermal_curtailed_series_mmbtu_per_hour"]) > 1000.0
                ...
                @test occursin("CHP.serve_absorption_chiller_only is set to true, but no months are specified.  All months will be enforced.", string(results["Messages"]["warnings"]))
                @test occursin("No techs are compatible with the provided or default heating load input for AbsorptionChiller, so the heating load input has been changed to SpaceHeating.", string(results["Messages"]["warnings"]))
                finalize(backend(m))
                empty!(m)
                GC.gc()
        ...
        @testset "PV outage production fraction" begin
            # Three fixed-size PVs at a flat 0.5 production factor: derated, full, and fully unavailable.
            # Sizes differ so that applying the derate to the wrong PV changes the expected total.
            d = Dict(
                ...
                "PV" => [
                    ...
                    Dict("name" => "full", "min_kw" => 50.0, "max_kw" => 50.0,
                         "production_factor_series" => fill(0.5, 8760)),
                    Dict("name" => "zero", "min_kw" => 40.0, "max_kw" => 40.0,
                         "production_factor_series" => fill(0.5, 8760),
                         "outage_production_fraction" => 0.0)
                ],
            ...
            # the derate is mapped per PV name and leaves other techs alone
            factors = REopt.outage_effective_production_factors(p)
            @test all(factors["derated"] .≈ 0.5 * 0.4)
            @test all(factors["full"] .≈ 0.5)
            @test all(factors["zero"] .≈ 0.0)
            ...
            @test all(collect(p.production_factor["zero", :].data) .≈ 0.5)

            m = Model(optimizer_with_attributes(HiGHS.Optimizer, "output_flag" => false, "log_to_console" => false, "presolve" => "on"))
            r = run_reopt(m, p)

            # Outages results sum over PVs, so check the total against each array's own expected output
            expected_pv_kw = 100.0 * 0.5 * 0.4 * p.levelization_factor["derated"] +
                              50.0 * 0.5 * p.levelization_factor["full"] +
                              40.0 * 0.5 * 0.0 * p.levelization_factor["zero"]
        ...
        @testset "Heating and cooling inputs + CHP defaults" begin
            ...
            @test inputs.s.chps[1].om_cost_per_kwh ≈ 0.027 atol=0.0001
            ...
            @test inputs.s.chps[1].om_cost_per_kwh ≈ 0.023 atol=0.0001
            ...
            @test inputs.s.chps[1].om_cost_per_kwh ≈ 0.014999999999999999 atol=0.0001
            ...
            @test inputs.s.chps[1].om_cost_per_kwh ≈ (0.75*0.015) atol=0.0001
        ...
        @testset "Emissions and Renewable Energy Percent" begin
            ...
            for i in range(1, stop=3)
                ...
                inputs["Site"]["renewable_electricity_min_fraction"] = if isnothing(RE_target[i])
                    0.0
                else
                    RE_target[i]
                end
        ...
        @testset "CST + High-temperature TES" begin
            ...
            m1 = Model(optimizer_with_attributes(HiGHS.Optimizer, "output_flag" => false, "log_to_console" => false, "mip_rel_gap" => 0.01))
            results = run_reopt(m1, p)
            @test results["CST"]["annual_thermal_production_mmbtu"] ≈ 6656.36 rtol=1.0e-3
            @test sum(results["CST"]["thermal_curtailed_series_mmbtu_per_hour"]) ≈ 19708.99 rtol=1.0e-3
            @test results["CST"]["size_kw"] ≈ 10000.0 atol=0.001
            @test results["CST"]["size_mmbtu_per_hour"] ≈ 10000.0 / REopt.KWH_PER_MMBTU atol=1.0e-3
            @test results["ExistingBoiler"]["annual_thermal_production_mmbtu"] ≈ 356.56 rtol=1.0e-3
        ...
        @testset "Battery O&M Cost Fraction" begin
            ...
            @test isapprox(year_one_om / init_capital_costs, 0.04; atol=0.0005)
            finalize(backend(m1))
            empty!(m1)
            finalize(backend(m2))
            empty!(m2)
