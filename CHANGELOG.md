...
### Fixed
- `data/chp/chp_cost_curve.json`, `data/chp/chp_defaults.json` Corrected the **micro_turbine** `size_class` 0 low-size cost point from 4900.0 to 5050.0 \$/kW. `size_class` 0 spans the full size range for a `prime_mover`, so its two cost points should match the smallest size class's low-size cost and the largest size class's high-size cost. **micro_turbine** `size_class` 1 (65-200 kW) starts at 5050.0 \$/kW, but `size_class` 0 (65-1000 kW) started at 4900.0 \$/kW. The other three prime movers already matched on both endpoints. The corresponding single-value **micro_turbine** `size_class` 0 default in `chp_defaults.json` changes from 4200.0 to 4275.0 \$/kW.
